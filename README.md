...
This project provides a Docker container for [FreeFileSync](https://freefilesync.org).
...
> [!NOTE]
> This Docker container is entirely unofficial and not made by the creators of FreeFileSync.
...
[Releases]: https://github.com/jlesage/docker-freefilesync/releases
[Docker Hub]: https://hub.docker.com/r/jlesage/freefilesync/tags
...
container, it would proxy all HTTP requests for `freefilesync.domain.tld` to
...
server {
	...
	location /freefilesync/ {
		...
		# Needed when audio support is enabled.
		location /freefilesync/websockify-audio {
			proxy_pass http://docker-freefilesync/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of FreeFileSync integrated into Docker image. |
...
Any desktop notification generated by FreeFileSync is forwarded to the
